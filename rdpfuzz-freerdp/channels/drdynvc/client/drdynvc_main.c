...
//#include <freerdp/channels/drdynvc_def.h>
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static void dvcman_send_create_process_pdu(IWTSVirtualChannelManager* pChannelMgr, char *ChannelName)
{
	WLog_INFO("FUZZ", "[dvcman_send_create_process_pdu] ChannelName=%s", ChannelName);

	UINT status;

	DVCMAN* dvcman = (DVCMAN*)pChannelMgr;
	drdynvcPlugin* drdynvc = dvcman->drdynvc;
	UINT32 ChannelId = rand();
	INT cbChannelId = ChannelId > 0xFFFFu ? 4 : ChannelId > 0xFFu ? 2 : 1;

	size_t MessageSize = cbChannelId + 1 + strlen(ChannelName);
	BYTE Message[MessageSize];

	wStream* data = Stream_New(Message, MessageSize);

	Stream_Write_UINT8(data, (CREATE_REQUEST_PDU << 4) | cbChannelId);
	switch (cbChannelId) 
	{
		case 1:
			Stream_Write_UINT8(data, (UINT8)ChannelId);
			break;
		case 2:
			Stream_Write_UINT16(data, (UINT16)ChannelId);
			break;
		case 4:
		default:
			Stream_Write_UINT32(data, ChannelId);
			break;
	}
	Stream_Write(data, ChannelName, strlen(ChannelName));

	WLog_INFO("FUZZ", "[dvcman_send_create_process_pdu] Command=%s", Stream_Buffer(data));

	status = drdynvc_send(drdynvc, data);

	if (status != CHANNEL_RC_OK)
	{
		WLog_INFO("FUZZ", "[drdynvc_process_create_request] VirtualChannelWriteEx failed with %s [%08" PRIX32 "]",
...
static IWTSVirtualChannelManager* dvcman_new(drdynvcPlugin* plugin)
{
	...
	dvcman->iface.CreateChannel = dvcman_send_create_process_pdu;
...
static void dvcman_channel_free(void* arg)
{
	...
	if (channel)
	{
		...
		WLog_INFO("FUZZ", "[dvcman_write_channel] critical section %p deleted", (LONGLONG)&(channel->lock));
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
UINT dvcman_write_channel(IWTSVirtualChannel* pChannel, ULONG cbSize, const BYTE* pBuffer,
                                 void* pReserved)
{
	BOOL close = FALSE;
	UINT status;
	...
	if (fuzzer_params.inside_fuzz_msg)
	{
		WLog_INFO("FUZZ", "[dvcman_write_channel] entering critical section %p", (LONGLONG)&(channel->lock));
	}
                                  ...
                                  UINT32 ChannelId, const char* ChannelName)
{
	...
	if (!fuzzer_params.loaded_fuzzer_parameters)
	{
		load_fuzzer_parameters();
	}
	...
	WLog_INFO("FUZZ", "ChannelName=%s target_channel_name=%s\r\n", ChannelName, fuzzer_params.channel_name);
...
fail:
	...
	if (strcmp(ChannelName, fuzzer_params.channel_name) == 0)
	{
		WLog_INFO("FUZZ", "channel \"%s\" found! address=%p \r\n", fuzzer_params.channel_name, channel);
		WLog_INFO("FUZZ", "[dvcman_write_channel] critical section %p created", (LONGLONG)&(channel->lock));
		fuzzer_params.target_channel = (IWTSVirtualChannel *)channel;

		fuzzer_params.initialized = true;
...
static UINT drdynvc_process_create_request(drdynvcPlugin* drdynvc, int Sp, int cbChId, wStream* s)
{
	...
	WLog_INFO("FUZZ", "[drdynvc_process_create_request] ChannelName=%s", name);
	...
	{
		WLog_INFO("FUZZ", "[drdynvc_process_create_request] ChannelName=%s created!", name);
		WLog_Print(drdynvc->log, WLOG_DEBUG, "channel created");
		Stream_Write_UINT32(data_out, 0);
	}
	else
	{
		WLog_INFO("FUZZ", "[drdynvc_process_create_request] ChannelName=%s no listener!", name);
...
static UINT drdynvc_order_recv(drdynvcPlugin* drdynvc, wStream* s)
{
	...
	switch (Cmd)
	{
		...
		// case 0xFFFFFFFF:
		// 	dvcman_send_create_process_pdu(NULL, NULL);
		// 	return ERROR_INTERNAL_ERROR;
