...
HANDLE case_written_event = NULL;
...
static void remove_shm(void) {
	...
	printf("tickcount=%ld closed shared memory2\r\n", GetTickCount());
...
static void setup_shm(void) {
  ...
  while(attempts < 5) {
	...
	printf("tickcount=%ld created shared memory, name=%s", GetTickCount(), shm_str);
...
BOOL module_loaded_to_pid(u32 pid, char * module_name, BOOL log)
{
	HMODULE hMods[1024];
	HANDLE hProcess;
	DWORD cbNeeded;
	...
	for (i = 0; i < (cbNeeded / sizeof(HMODULE)); i++)
	{
		...
		if (GetModuleFileNameExA(hProcess, hMods[i], mod_path, sizeof(mod_path)))
		{
			// check if this is the module we are after
			mod_name = strrchr(mod_path, '\\');
			...
			if (mod_name)
			{
				if (stricmp(mod_name + 1, module_name) == 0)
				{
					found = TRUE;
					break;
				}
...
u32 find_pid_by_loaded_module(char * module_name)
{
	u32 attach_pid = 0;
	...
	do
	{
		...
		{
			if (aProcesses[i] == 2664)
			{
				log = TRUE;
			}
			if (module_loaded_to_pid(aProcesses[i], module_name, log))
			{
				found = TRUE;
				attach_pid = aProcesses[i];
				break;
			}
		}
	} while (!found && drattachwait);
	return attach_pid;
}
...
static void create_target_process(char** argv) {
  ...
  if(drioless) {
    ...
    ck_free(static_config);
	  ...
	  if (drattach)
	  {
		  //// let the new svchost start
		  //Sleep(1000); 

		  //// patch the new svchost
		  //CreateProcessA(NULL, "C:\\RDPFuzz\\bin\\tools\\mimikatz\\x64\\mimikatz.exe privilege::debug ts::multirdp exit",
			 // NULL, NULL, FALSE, 0, NULL, NULL, &si2, &pi2);

		  if (drattachpid == 0)
		  {
			  drattachpid = find_pid_by_loaded_module(drattach_module_name);
		  }
		  cmd = alloc_printf(
  ...
  printf("-----About to run drrun process cmdline=%s", cmd);
...
char ReadCommandFromPipe(u32 timeout)
{
	...
	DWORD bytes_written;
	HANDLE hfile = CreateFileA("C:\\RDPFuzz\\Logs\\afl-fuzz.txt", FILE_ALL_ACCESS, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if (hfile != INVALID_HANDLE_VALUE)
	{
		char log_msg[1024];
		sprintf_s(log_msg, 1024, "read from pipe '%c'\r\n", result);

		SetFilePointer(hfile, 0, NULL, FILE_END);
		WriteFile(hfile, log_msg, strlen(log_msg), &bytes_written, NULL);
...
void WriteCommandToPipe(char cmd)
{
	...
	{
		char log_msg[1024];
		sprintf_s(log_msg, 1024, "wrote to pipe '%c'\r\n", cmd);
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (!case_written_event)
  {
	  char *id_str = strstr(out_file, "input\\") + 6;
	  sprintf_s(event_name, 1000, "Global\\case_event_fuzz%s", id_str);
	  event_name[24] = 0;
	  case_written_event = CreateEventA(NULL, FALSE, FALSE, event_name);
	  if (case_written_event == NULL)
	  {
		  printf("failed to create event %s\r\n", event_name);
	  }
	  else
	  {
		  printf("created event %s last_error=%08x\r\n", event_name, GetLastError());
	  }
  }
  SetEvent(case_written_event);

  //printf("set fuzz event\r\n");
...
static void write_to_testcase(void* mem, u32 len) {
  ...
  if (out_file) {
    ...
    if (fd < 0) {
		...
		CHAR log_msg[1000];
		sprintf_s(log_file_name, 1000, "C:\\RDPFuzz\\Logs\\testcase-write-fails-%s.txt", fuzzer_id);
		HANDLE hfile = CreateFileA(log_file_name, FILE_ALL_ACCESS, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		if (hfile != INVALID_HANDLE_VALUE)
		{
			sprintf_s(log_msg, 1000, "failed to write file (first try) - going to kill the target\r\n", fuzzer_id);
			SetFilePointer(hfile, 0, NULL, FILE_END);
			WriteFile(hfile, log_msg, strlen(log_msg), &bytes_written, NULL);
...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ afl options ] -- [instrumentation options] -- "
	   ...
	   "  -a pid           - attach to a process with the given pid\n"
	   "  -A module_name   - attach to a process that loaded the given module\n\n"

	   "Background fuzzing (marker) settings:\n\n"

	   "  -data_param_number num       - number of param holding the data to the target function\n"
	   "  -data_size_param_number num  - number of param holding the data size to the target function\n"
	   "  -marker_offset num           - offset in bytes where the marker was inserted\n"
	   "  -marker num                  - 8-bit. if the value at the marker offset exceeds it, it is a fuzz msg\n"
	   "  -last_msg_marker num         - 8-bit. if the value exceeds marker+last_msg_marker, it is the last of the sequence\n\n"

	   "  -fuzz_log                    - turn fuzz log on \n"
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:I:T:dYnCB:S:M:x:QD:b:l:pPc:a:A:w:")) > 0)
